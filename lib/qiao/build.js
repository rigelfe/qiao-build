...
cli.main = function ( args, opts ) {
    ...
    // include 开始
    // 增加qiao-build目录
    var buildDir = path.resolve(inputDir, 'qiao-build');
    if (fs.existsSync(buildDir)) {
        if (opts.force) {
            require( '../util' ).rmdir( buildDir );
        }
        else {
            console.error( buildDir + ' directory already exists!' );
            return;
        }
    }
    var util = require('../util');
    require('mkdirp').mkdirp(buildDir);
    
    // 将include的文件复制到qiao-build目录
    ...
    include.forEach(function (item) {
        ...
        items = items.concat(processPaths(path.resolve(inputDir, item)));
        items.forEach(function (o) {
            item = path.relative(inputDir, o);
            util.copy(o, path.resolve(buildDir, item));
        });
    });

    // 复制module.conf到build目录
    var modulePath = path.resolve(inputDir, 'module.conf');
    util.copy(modulePath, path.resolve(buildDir, path.relative(inputDir, modulePath)));
    // 复制combine.conf到build目录
    var combinePath = path.resolve(inputDir, 'combine.conf');
    util.copy(combinePath, path.resolve(buildDir, path.relative(inputDir, combinePath)));
    // 复制edb-build-config.js到build目录
    var configPath = path.resolve(inputDir, 'edp-build-config.js');
    util.copy(configPath, path.resolve(buildDir, path.relative(inputDir, configPath)));

    // 更改构建的输入和输出目录
    // 装载构建配置模块
    var conf = loadConf( opts.config, buildDir );
    outputDir && (conf.output = path.resolve( process.cwd(), outputDir ));
    outputDir = conf.output;
    inputDir = conf.input = path.resolve( process.cwd(), buildDir);
    // include end
    ...
    conf.injectProcessor({
        createProcessor: require('../processor')
    });
